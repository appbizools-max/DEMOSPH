...
const isMatchingBranch = (app: any, activeBranch?: string) => {
  if (!activeBranch || activeBranch === 'All Branches') return true;
  ...
  const isBranchMatching = (b1: string, b2: string): boolean => {
    const n1 = b1.toLowerCase().replace(/\s*branch\s*/i, '').trim();
    const n2 = b2.toLowerCase().replace(/\s*branch\s*/i, '').trim();
    ...
    return getShortcut(n1) === getShortcut(n2);
  };
...
const mapRecord = (app: any, index: number, selectedDate: string): PatientAppointmentRecord => {
  ...
  if (s === 'completed' || s === 'done' || s === 'paid' || p === 'paid') {
    status = 'completed';
  } else if (s === 'collect_fee' || app.feeCollectionNeeded === true || (s === 'active' && p === 'pending')) {
    status = 'collect_fee';
  } else if (s === 'active' || s === 'in_consultation' || s === 'in-clinic' || s === 'in-consultation') {
    status = 'active';
  } else {
    status = 'upcoming';
  }
    ...
    <View style={styles.patientCard}>
      ...
      <View style={styles.actionRow}>
        {patient.status !== 'completed' && (
          <TouchableOpacity
            style={[styles.rescheduleIconBtn, { backgroundColor: '#e0f2fe' }]}
            onPress={() => onNavigate && onNavigate('reception_patient_file', patient)}
          >
            <MaterialCommunityIcons name="folder-account-outline" size={18} color="#0284c7" />
          </TouchableOpacity>
        )}
...
export const ReceptionDashboardScreen: React.FC<ReceptionDashboardScreenProps> = ({ currentBranch, onNavigate }) => {
  ...
  useEffect(() => {
    if (currentBranch) {
      const def = getBranchDefaultTarget(currentBranch);
      setBranchTarget({
  ...
  useEffect(() => {
    try {
      ...
      const unsubscribe = onSnapshot(colRef, (snapshot) => {
        const curBranch = currentBranch || branchTarget.branchName || 'KPHB Branch';
  ...
  }, [currentBranch, branchTarget.branchName]);
  ...
  // Subscribe to real-time Firestore appointments & allpatients (Filtered by active branch)
  useEffect(() => {
    ...
    const activeBranch = currentBranch || branchTarget.branchName || 'KPHB Branch';
        ...
        if (combinedMap.has(item.id)) {
          ...
          if (cleanPhone && date && time) {
            for (const existing of combinedMap.values()) {
              const exPhone = String(existing.phone || existing.phoneNumber || '').replace(/\D/g, '').slice(-10);
    ...
    try {
      unsubApp = onSnapshot(collection(db, 'appointments'), (snapshot) => {
        ...
        snapshot.forEach((snap) => {
          ...
          if (isMatchingBranch(data, activeBranch)) {
            list.push({ id: snap.id, collectionName: 'appointments', ...data });
          }
        });
    ...
    try {
      unsubPat = onSnapshot(collection(db, 'allpatients'), (snapshot) => {
        ...
        snapshot.forEach((snap) => {
          const data = snap.data();
          if ((data.appointmentDate || data.date || data.appointmentTime || data.doctor || data.status) && isMatchingBranch(data, activeBranch)) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const targetApp = liveAppointments.find(a => a.id === patientId);
              ...
              await deleteDoc(doc(db, 'appointments', patientId)).catch(() => { });
              await deleteDoc(doc(db, 'allpatients', patientId)).catch(() => { });
              await deleteDoc(doc(db, 'patients', patientId)).catch(() => { });

              // Deep clean duplicate records by phone + name across all collections
              if (pPhone || patientName) {
  ...
  const activeBranch = currentBranch || branchTarget.branchName;
  ...
  const dashboardMetrics = useMemo(() => {
    ...
    filteredAppointments.forEach(app => {
      ...
      if (st === 'waiting' || st === 'scheduled' || st === 'upcoming' || st === 'pending') {
        waiting++;
      } else if (st === 'active' || st === 'in_consultation' || st === 'in-consultation') {
        activeConsultationsCount++;
      } else if (st === 'completed' || st === 'done' || st === 'finished') {
        completed++;
      }

      if (app.paymentStatus === 'pending' || app.paymentPending === true) {
        payPending++;
      }

      if (app.followup === true || app.followUpOpted === true) {
        followupOpted++;
      }
    });
  ...
  const upcomingList = useMemo(() => {
    return filteredAppointments
      .filter(app => {
        const s = String(app.status || '').toLowerCase().trim();
        const p = String(app.paymentStatus || '').toLowerCase().trim();
        const isCompleted = s === 'completed' || s === 'done' || s === 'paid' || p === 'paid';
        const isActive = s === 'active' || s === 'in_consultation' || s === 'in-consultation' || s === 'collect_fee' || app.feeCollectionNeeded === true;
      ...
      .map((app, idx) => mapRecord(app, idx, selectedDate));
  ...
  const completedList = useMemo(() => {
    return filteredAppointments
      .filter(app => {
        const s = String(app.status || '').toLowerCase().trim();
        const p = String(app.paymentStatus || '').toLowerCase().trim();
        return s === 'completed' || s === 'done' || s === 'paid' || p === 'paid';
      })
      .map((app, idx) => mapRecord(app, idx, selectedDate));
  }, [filteredAppointments, selectedDate]);
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.metricsRow}>
        ...
        <View style={[styles.metricCard, { borderTopColor: '#d97706' }]}>
          <Text style={[styles.metricNum, { color: '#d97706' }]}>{dashboardMetrics.waiting}</Text>
        ...
        <View style={[styles.metricCard, { borderTopColor: '#16a34a' }]}>
          <Text style={[styles.metricNum, { color: '#16a34a' }]}>{dashboardMetrics.completed}</Text>
      ...
      <View style={styles.metricsRowSecond}>
        <View style={[styles.metricCardWide, { borderTopColor: '#16a34a' }]}>
          <Text style={[styles.metricNum, { color: '#16a34a' }]}>{dashboardMetrics.apptsCompleted}</Text>
